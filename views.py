...
class LoginPage(tk.Frame):
    ...
    def create_widgets(self):
        ...
        tk.Label(login_frame, text="Inloggen", font=("Arial", 20),
        fg="white",bg="#222").grid(row=0, column=0, columnspan=2,
        pady=10)
        tk.Label(login_frame, text="Gebruikersnaam:", font=("Arial", 14),
        fg="white",bg="#222").grid(row=1, column=0, sticky="e", padx=10)
        self.gebruikersnaam_entry = tk.Entry(login_frame, width=25)
        self.gebruikersnaam_entry.grid(row=1, column=1, padx=10, pady=5)
        tk.Label(login_frame, text="Wachtwoord:", font=("Arial", 14),
        fg="white",bg="#222").grid(row=2, column=0, sticky="e", padx=10)
        ...
        bg="#444",fg="white").grid(row=3, column=0, columnspan=2,
        pady=10)
    ...
    def login(self):
        ...
        if (gebruikersnaam in self.studenten and
            ...
            messagebox.showerror("inloggen mislukt",
            "Onjusite gebruikersnaam of wachtwoord.")
...
class StudentDashboard(tk.Frame):
    ...
    def create_widgets(self):
        ...
        tk.Label(self, text=f"Welkom, {self.student.voornaam}!",
        font=("Arial", 20), fg="white", bg="#222").pack(pady=10)
        tk.Label(self, text=f"Studiepunten: {total_points}/60",
        font=("Arial", 16), fg="white", bg="#222").pack()
        tk.Label(self,
        text=f"Je hebt {remaining_points} "
             f"meer studiepunten nodig om je propedeuse te halen.",
        font=("Arial", 14), fg="white", bg="#222").pack(pady=10)

        tk.Label(self, text="Jou examen:", font=("Arial", 16),
        fg="white", bg="#222").pack()

        for grade in self.student.grades:
            frame = tk.Frame(self, bg="#333", bd=2,
            ...
            g=grade: self.show_grade_details(g))

        tk.Button(self, text="Log uit",command=lambda:
        self.master.switch_frame(LoginPage, self.master.studenten,
        self.master.docenten),bg="#444", fg="white",
...
class DocentDashboard(tk.Frame):
    ...
    def create_widgets(self):
        tk.Label(self, text="Docent Dashboard",
        font=("Arial", 20), fg="white", bg="#222").pack(pady=10)

        student_names = [f"{s.voornaam} {s.achternaam}"
        for s in self.studenten.values()]
        self.student_selection = ttk.Combobox(self,
        values=student_names, state="readonly")
        self.student_selection.pack(pady=10)
        self.student_selection.bind("<<ComboboxSelected>>",
        self.load_student_grades)

        self.grades_frame = tk.Frame(self, bg="#222")
        self.grades_frame.pack(pady=10, fill="both", expand=True)

        tk.Button(self, text="Nieuwe Examen",
        command=self.add_new_grade, bg="#444", fg="white").pack(pady=10)
        tk.Button(self, text="Log uit",
        command=lambda: self.master.switch_frame(LoginPage,
        self.studenten, self.master.docenten),
    ...
    def load_student_grades(self, _):
        ...
        self.selected_student = next((s for s in
        self.studenten.values() if
        f"{s.voornaam} {s.achternaam}" == selected_student_name), None)
        ...
        for grade in self.selected_student.grades:
            ...
            (side="right", padx=5))
            (tk.Button(frame, text="Verwijder", command=lambda g=grade:
            self.delete_grade(g), bg="#555", fg="white").pack
            (side="right"))
    ...
    def grade_form_popup(self, grade=None):
        ...
        tk.Label(popup, text=f"Student: "
        f"{self.selected_student.voornaam} "
        f"{self.selected_student.achternaam}", font=("Arial", 14),
        fg="white", bg="#222").pack(pady=10)

        tk.Label(popup, text="Toets:", font=("Arial", 12),
        fg="white", bg="#222").pack(pady=5)
        subject_entry = tk.Entry(popup, width=30)
        subject_entry.pack(pady=5)

        tk.Label(popup, text="Cijfer:", font=("Arial", 12),
        fg="white", bg="#222").pack(pady=5)
        ...
        tk.Label(popup, text="Pogingen:", font=("Arial", 12),
        fg="white", bg="#222").pack(pady=5)
        ...
        def save():
            ...
            try:
                ...
                "Cijfer moet een getal zijn tussen de 1-10.")
                return
            ...
            try:
                ...
                messagebox.showerror("Error",
                "Geen geldige datum input. gebruik dd-mm-yyyy.")
                return

            attempts = attempts_entry.get()
            if not attempts.isdigit() or int(attempts) <= 0:
                messagebox.showerror("Error",
                "Pogingen moeten een positieve integer zijn")
                return
            ...
            if grade:
                grade.update({
                    ...
                    .strip(),
                    "status": "Behaald" if grade_val >= 5.5
                    else "Niet behaald",
                })
            else:
                self.selected_student.add_grade(subject,
        ...
        tk.Button(popup, text="Opslaan", command=save, bg="#444",
